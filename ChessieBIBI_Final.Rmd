...
date: <h4 style="font-style:normal">January 22, 2019</h4>
author:
...
Setting up the data tables and establishing a connection to the analysis database will likely require some time. Once this is accomplished, implementing the scripts should be a fairly quick process.    

RStudio must be installed on your computer to implement the folloring R scripts. If RStudio is not installed, it can be downloaded for free from [https://www.rstudio.com/](https://www.rstudio.com/). This [webpage](https://courses.edx.org/courses/UTAustinX/UT.7.01x/3T2014/56c5437b88fa43cf828bff5371c6a924/) details the short installation process. If RStudio is already installed, check which version is installed (Help -> About RStudio). The scripts to calculate the Chessie BIBI were written in Version 1.1.453, so if you have an older version, we urge you to upgrade (Help -> Check for Update -> follow instructions). Currently, the directions for connecting to the database are written for a Windows machine.      

The R scripts in this document are designed to be copied and pasted into the RStudio editor. In order to keep the scripts organized, it is suggested that a header is added using `# text` for each code chunk corresponding to these instructions. For example, for the first step of loading packages, type in `# 1.1` as the header. Also, each time a data frame is created, it can be viewed by typing `View(name)` in the console or by clicking its name in the Environment tab on the right.          

For those who are new to RStudio, please refer to [this](http://www.sthda.com/sthda/RDoc/images/rstudio.png) image for the layout of the RStudio panels. Some helpful R books and webpages are listed below:

* The R Book by Michael J. Crawley  
* R for Data Science by Hadley Wickham and Garrett Grolemund (PDF: https://r4ds.had.co.nz/index.html)
...
###**1. Data Preparation**   
Most macroinvertebrate assemblage data are formatted in a similar manner, but data sets from separate entities often cannot be joined without some manipulation. The data preparation steps below manipulate the data to meet formatting requirements necessary for the BIBI calculation steps. The formatting is similar to that of the Chesapeake Environmental Data Repository (CEDR) database at the Chesapeake Bay Program (CBP) [Data Hub](https://www.chesapeakebay.net/what/data), which is coordinating with the national [Water Quality Portal (WQP)](https://www.waterqualitydata.us/).      

You will need to have your data in an MS Access database. It needs to contain the follow tables:   
...
Please refer to the (name and url) document and (name and url) practice data set for information about setting up the tables above. Data may be entered in uppercase or lowercase, but all text will be converted to lowercase during the data preparation process. The document also contains several keys that explain codes and acronyms that were used. The codes and acronyms can be used as a template for your data tables. Some of the information that the codes pertain to can be found in domain lists on the bottom of the [Water Quality Portal User Guide](https://www.waterqualitydata.us/portal_userguide/) page. The lists that may be useful are Habit, RelativeDepth, AnalyticalMethod, FrequencyClassDescriptor, MeasurementUnit, SampleCollectionEquipment, and Taxon. It is very important that you name all the columns and tables in the same way as they appear in the document, or the scripts will not work.           

####**1.1** 
Open RStudio. Create a new RScript (File > New File > R Script), and save it in a location where you would like the scripts and CSV files to be saved.      

In the code editor (RStudio's upper left quadrant), use `setwd(" ")` to set the working directory to the location where you saved your R Script file. For example, `setwd("C:/Documents/My folder")`. Don't forget to use a forward slash `/` instead of a backward slash `\` in the path name. This is also the location where any CSV files you generate will be saved.  

Load the packages below into the environment. The packages must be installed before they can be opened. To check if a package is already installed, go to the Packages tab (RStudio's lower right quadrant) and either scroll to find it or type in the name in the search bar. To install a package, either use `install.packages("name")` or the Install button in the Packages tab. To open an installed library, use `library(name)`.    

*bibi2.0* is a custom package developed by Zachary Smith and Rikke Jepsen, and functions from it will be used to automate the data preparation process. To download this package, you will need to have the `devtools` package installed. Then run the following script: `devtools::install_github("InterstateCommissionPotomacRiverBasin/bibi2.0")` and load using the scripts below. Install and load all of the following packages, so that you will not have to stop mid-script to load a package.    
...
```
####**1.2**
Import the tables below from your Microsoft Access database. An Open Database Connectivity (ODBC) interface is used to link the MS Access database to RStudio. This process will differ depending on whether your computer uses a 64-bit or 32-bit Windows operating system and Microsoft Office. If you are unsure of which Windows operating system your computer has, follow the directions on [this](https://support.microsoft.com/en-us/help/13443/windows-which-operating-system) site to check. Refer to [this](https://support.office.com/en-us/article/about-office-what-version-of-office-am-i-using-932788b8-a3ce-44bf-bb09-e334518b8b19) site to determine which version of Microsoft Office is installed.      

If you need help establishing the ODBC connection, refer to [this](https://www.youtube.com/watch?v=Uh0pLPS3R5Y) YouTube video.   
...
2. Once the MS Access Database Engine is installed, press the "Windows key" + "R", or go to Start > Run and type the following into the Run dialog: `c:\windows\sysWOW64\odbcad32.exe`
3. You can now go back to the ODBC Data Source Administrator and proceed with adding your database.  
4. The last step is to change RStudio to the 32-bit version. Go to Tools > Global Options > General > change the version to 32-bit.  

**NOTE** The database used in the instructions below is named CBIBI_2017, but, as yours will no doubt have a different name, change the code below from `channel <- dbConnect(odbc(), "CBIBI_2017")` to `channel <- dbConnect(odbc(), "your_database_name")`.   

Windows 64-bit with MS Office 64-bit or 32-bit:
```{r}
...
```
Windows 32-bit with MS Office 32-bit:
```{r eval=FALSE}
...
RODBC::odbcCloseAll()
```
Once the ODBC connection is established and the tables from your databse have been imported, the *bibi2.0* function, `clean_data_frame()`, is implemented. This sets all the text entries to lowercase and removes any spaces before and after a data entry.  


####**1.3**
Join the Master Taxa List to the taxonomic count table using the *tsn_final* column. Then, apply the `prep_taxa()` function from the *bibi2.0* package to perform the taxa exclusions and roll-ups described on pages 6-7 in Smith et al. (2017). This step also creates a new variable, unique_id, which is a combination of event_id and sample_num. The unique_id will be used extensively in the rest of the scripts. 
```{r}
...
```
####**1.4**
Perform method standardizations as described on page 5 of Smith et al. (2017) by applying the `prep_data()` function from the *bibi2.0* package to the indicated tables. Several functions are nested within `prep_data()`, including:  

1) Water quality prep function: select only specific conductivity, dissolved oxygen, and pH;  

2) Habitat prep function: select only epifaunal substrate, bank stability, embeddedness, channel alteration, riffle/run/pool/ ratio, flow, bank vegetation, and sedimentation, then calculate habitat conditions (produces na_hab, hab_6, hab_12, and hab_16 columns);   

3) Method prep function: exclude samples collected from December to February, include only samples from 1^st^ - 4^th^ order streams, and restrict the sampling method to kick-net or similar procedures.  
The output is **prep.df**, one large data frame which contains all of the taxa, water quality, and habitat data. The water quality and habitat data are not necessary for calculating the Chessie BIBI, but the prep functions in the *bibi2.0* package are nested such that you cannot comment those steps out.     
```{r}
...
```
####**1.5**
Convert the Functional Feeding Group (FFG) and Habit codes to full names. This adds two new columns, *ffg* and *habit*, to **prep.df2**, which will be used instead of *bibi_ffg* and *bibi_habit* in later scripts.
```{r}
prep.df2 <-
...
taxa.df <- prep.df2 %>% 
  ungroup() %>% 
  select(unique_id, icprb_bioregion_id, reporting_value, final_id, phylum, subphylum, class, subclass, 
...
```
####**1.7** 
Perform probabilistic rarefaction as described in Smith et al. (2017) page 7 and Appendix D.  

The Chessie BIBI uses rarefied counts of 100 because that was the lowest common denominator among all the agencies and programs represented in the Chessie BIBI database. Changing the sample size to a number other than 100 will alter the final outcome, but if you decide to change the sample size, substitute your number for 100 in the 4^th^ line of code below: `mutate(ratio = YOUR_NUMBER*(reporting_value/sample_sum)) %>% `.  

**WARNING:** every time you rerun the rarefaction scripts below, the resulting data frame will have different rarefied counts for rare species (usually the singletons or doubletons in your dataset). To preserve the data frame generated by rarefaction, **taxa.df**, export it as a CSV file. If you need to stop and close the document now, import the rarefied dataset CSV table prior to the next step, which calculates the biological metrics for the Chessie BIBI index.
```{r}
taxa2.df <- taxa.df %>%                                                                      
  group_by(unique_id) %>%                                                                               
...
```
**Optional:**  
If you would like to preserve the rarefied data frame created above, export it as a CSV. The CSV will be stored in the working directory that you have specified. 
```{r eval=FALSE}
write.csv(taxa.df, file= "taxa.df.csv")
```
###**2. Calculating the Biological Metrics**  
Now that the data are in the proper format and the reporting values have been rarefied, the biological metrics can be calculated. The metric formulas are located in Smith et al. (2017) Appendix E. Please note that the appendix contains descriptions of all the metrics tested during the development of the Chessie BIBI, so includes many that did not eventually end up as part of the BIBI. Also, please note that if you downloaded Appendix E from [www.potomacriver.org](www.potomacriver.org) before 1/23/2019, you will have to download it again as several typos have been corrected.  

The scripts below compute for each unique sampling event (unique_id) all of the possible family-level metrics needed for the Chessie BIBI. Metrics specific to each bioregion are selected and used to calculate the bioregion-specific BIBI indices in a later step.      

The first step is to generate a new data frame for the non-rarefied and rarefied data, both of which are needed because not all metric calculations require rarefied data. Then, a data frame, **metrics.key**, will be generated. At first, **metrics.key** only contains the following columns: unique_id, icprb_bioregion_id, event_latitude, event_longitude, and huc_12, but each calculated metric is appended using the `$` notation; for example `metrics.key$new_column <-`.
...
# If you had exported the rarefied data as a CSV above, read it in at this time using the script below. Don't forget to uncomment the script by removing the #. 
...
aspt_mod <- non_rare %>%
 ...
 summarise(aspt_mod = mean(bibi_tv))
 metrics.key$aspt_mod <- aspt_mod$aspt_mod

# pct_gastro_oligo 
pct_gastro_oligo <- non_rare %>%
 group_by(unique_id) %>%
 filter(!is.na(class)) %>% 
 summarise(sum = sum(reporting_value[class %in% c("gastropoda", "oligochatea")]))  
...
pielou_taxa_rich <- family_richness %>%
 mutate(family_richness = log(family_richness))
...
rich_burrow <- rare  %>%
 ...
 metrics.key$rich_burrow[is.na(metrics.key$rich_burrow)] <- 0
...
rich_climb <- rare  %>%
 ...
 metrics.key$rich_climb[is.na(metrics.key$rich_climb)] <- 0
...
rich_cling <- rare  %>%
 ...
 metrics.key$rich_cling[is.na(metrics.key$rich_cling)] <- 0
...
rich_trichoptera <- rare  %>%
 ...
 metrics.key[is.na(metrics.key)] <- 0 
```
To download metrics.key as a CSV, run the following script. The CSV will be stored in the working directory that you have specified. Don't forget to replace `file= ` with your own file name.
```{r eval=FALSE}
write.csv(metrics.key, file= "name_of_file.csv")
```
###**3. Metric Scoring**  
####**3.1**
Subset **metrics.key** by bioregion to create twelve new data frames.
```{r}
...
```
####**3.2**
Create a data frame for each of the twelve bioregions that contains only the metrics specific to that bioregion. A list of the metrics used for each bioregion can be found in Table 15, Page 44 of Smith et al. (2017). One change has been made from the original list of metrics: pct_predator was removed from NAPU, as it was deemed unnecessary.  
```{r}
...
```
####**3.3**
Score the metrics using the approach described on pages 20-21 in Smith et al. (2017) and outlined below:    

1. Go to Smith et al. (2017) Table 15, Page 44. (Note: this table can also be [downloaded](https://www.potomacriver.org/wp-content/uploads/2014/12/metric_thresholds_06292017.csv) from the ICPRB website, but it will need to be filtered so that it only contains bioregion and family data.)    
...
3. On Smith et al. (2017) Page 21, select Equation 5 if the metric decreases with disturbance or Equation 6 if the metric increases with disturbance. 

4. Refer to Table 15 again. The *Reference Median* and *Bound* columns correspond to *X~M~* and *X~T~* in the equations, respectively. These scoring equations are implemented below.
```{r}
# NAPU Metric Scoring
...
The resulting mean value is the Chessie BIBI for each unique_id, and it is located in a new column called *chessie_bibi*, assigned using `bioregion_score$chessie_bibi <-`.
```{r}
...
```
###**5. Assigning Narrative Ratings** 
Use Table 5, Page 27 in Smith et al. (2017) to assign narrative ratings of Excellent, Very Good, Fair, Poor, or Very Poor to the Chessie BIBI values.
```{r}
NAPU_score$rating <-
  case_when(
...
```
To download a CSV of the Chessie BIBI ratings in all bioregions, use the following script. The CSV will be stored in the working directory that you have specified. Don't forget to replace `file= ` with your own file name.
```{r eval=FALSE}
write.csv(bibi_ratings_combined, file= "name_of_file.csv")
```
####**6.2**
The following script provides a quick way of visually checking your results. It creates a stacked bar chart of the ratings for each bioregion with `ggplot2`.
```{r echo=TRUE}
