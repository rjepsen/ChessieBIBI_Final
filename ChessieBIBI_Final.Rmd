...
```
**Optional:**  
If you would like to preserve the rarefied data frame created above, export it as a CSV. 
```{r eval=FALSE}
...
# Calculate the metrics. Use long.df to specify which data frame to use (non-rarefied or rarefied. First, the non-rarefied data will be used.)
 ...
 # metrics.key$aspt_mod2 <- taxa_tol_index(    
 #  long.df = non_rare,
...
margalefs <- rare %>%
   group_by(unique_id) %>%
   summarise(sample_sum = sum(rare_count))
metrics.key$pielou <- ((family_richness$family_richness - 1)/(log(margalefs$sample_sum))) # NOT APPEARING IN METRIC KEY WHY
   
  metrics.key$margalefs2 = taxa_div(
  long.df = rare,
  unique.id.col = unique_id,
...
##  PIELOU
  ...
  metrics.key$pielou <- (shannon$shannon / log(family_richness$family_richness))
      
  metrics.key$pielou2 <- taxa_div(
  long.df = rare,
  unique.id.col = unique_id,
  count.col = rare_count, 
  high.taxa.col = family,
...
##  
 ...
 rich_intol <- rare  %>%
    group_by(unique_id) %>%
    filter(!is.na(bibi_tv)) %>%
    filter(bibi_tv %in% c("0", "1", "2", "3")) %>%
    summarise(rich_intol = n_distinct(family)) %>%
    select(unique_id, rich_intol) 
    metrics.key <- merge(metrics.key, rich_intol, by = "unique_id", all=TRUE)
    metrics.key[is.na(metrics.key)] <- 0   

  metrics.key$rich_intol2 <- taxa_rich(
  long.df = rare,
  unique.id.col = unique_id,
 ...
 rich_modtol <- rare  %>%
    group_by(unique_id) %>%
    filter(!is.na(bibi_tv)) %>%
    filter(bibi_tv %in% c("4", "5", "6")) %>%
    summarise(rich_modtol = n_distinct(family)) %>%
    select(unique_id, rich_modtol) 
    metrics.key <- merge(metrics.key, rich_modtol, by = "unique_id", all=TRUE)
    metrics.key[is.na(metrics.key)] <- 0   
  
 metrics.key$rich_modtol2 <- taxa_rich(
  long.df = rare,
  unique.id.col = unique_id,
  low.taxa.col = bibi_tv,
  high.taxa.col = family,
 ...
 rich_sprawl <- rare  %>%
  ...
  rich_tol <- rare  %>%
    group_by(unique_id) %>%
    filter(!is.na(bibi_tv)) %>%
    filter(bibi_tv %in% c("7", "8", "9", "10")) %>%
    summarise(rich_tol = n_distinct(family)) %>%
    select(unique_id, rich_tol) 
    metrics.key <- merge(metrics.key, rich_tol, by = "unique_id", all=TRUE)
    metrics.key[is.na(metrics.key)] <- 0 
    
  metrics.key$rich_tol2 <- taxa_rich(
  long.df = rare,
  unique.id.col = unique_id,
  low.taxa.col = bibi_tv,
  high.taxa.col = family,
