...
```
**Optional:**  
If you would like to preserve the rarefied data frame created above, export it as a CSV. Don't forget to replace `file= ` with your own file name. 
```{r eval=FALSE}
write.csv(taxa.df, file= "name_of_file.csv")
```
...
The first step is to generate a new data frame for the non-rarefied and rarefied data, both of which are needed because not all metrics calculations require rarefied data. Then, a data frame, **metrics.key**, will be generated. **metrics.key** initially only contains the following columns: unique_id, icprb_bioregion_id, event_latitude, event_longitude, and huc_12, but each calculated metric is appended using the `$` notation; for example `metrics.key$new_column <-`.
...
rich_trichoptera <- rare  %>%
 ...
 metrics.key[is.na(metrics.key)] <- 0 
```
To download metrics.key as a CSV, run the following script. The CSV will be stored in the working directory that you have set up. Don't forget to replace `file= ` with your own file name.
```{r eval=FALSE}
...
```
To download a CSV of the Chessie BIBI ratings of all bioregions, use the following script. Don't forget to replace `file= ` with your own file name.
```{r eval=FALSE}
