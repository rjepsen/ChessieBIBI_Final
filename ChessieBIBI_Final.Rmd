...
The initial setup of the data tables and establishing a connection to the database will likely require a decent input of time. Once, however, this is accomplished, implementing the scripts should be a fairly quick process.      

In order to implement the following R scripts, RStudio must be installed. If RStudio is not installed, it can be downloaded for free from [https://www.rstudio.com/](https://www.rstudio.com/). This [webpage](https://courses.edx.org/courses/UTAustinX/UT.7.01x/3T2014/56c5437b88fa43cf828bff5371c6a924/) details the short installation process. If RStudio is already installed, check which version is installed (Help -> About RStudio). These scripts were written in Version 1.1.453, so if you have an older version, you may want to upgrade (Help -> Check for Update -> follow instructions). Currently, the directions for connecting to the database are written for a Windows machine.      

Once RStudio is installed, or if it is already installed on your computer, the scripts are designed to be copied and pasted into the RStudio editor. In order to keep the scripts organized, it is suggested that a header is commented out, using `# text`, for each code chunk corresponding to these instructions. For example, for the first step of loading packages, type in `# 1.1` as the header.        
...
###**1. Data Preparation**   
Most macroinvertebrate assemblage data are formatted in a similar manner; however, rarely can data sets from two separate entities be joined without some manipulation. The data preparation steps below manipulate the data to meet formatting requirements necessary for the BIBI calcualtion steps.     
...
####**1.1** 
Open a new RScript (File > New File > R Script) and save it.    

Set the working directory to the location where you saved your R Script file using `setwd(" ")`. For example, `setwd("C:/Documents/My folder")`. Don't forget to use a forward slash `/` instead of a backward slash `\` in the path name. This is also the location where any CSV files you generate will be saved.       

Load the packages below into the environment. The packages must be installed before they can be opened. To check if a package is already installed, go to the Packages tab on the right and either scroll to find it or type in the name in the search bar. To install a package, either use `install.packages("name")` or the Install button in the Packages tab. To open an installed library, use `library(name)`.    

*bibi2.0* is a custom package developed by Zachary Smith, and functions from it will be used to automate the data preparation process. To download this package you will need to have the `devtools` package installed. Then run the following script: `devtools::install_github("InterstateCommissionPotomacRiverBasin/bibi2.0")` and load using the scripts below.   
...
```
####**1.2**
Import the following tables from your Microsoft Access database. This process will differ depending on whether your computer uses a 64-bit or 32-bit Windows operating system and Microsoft Office. If you are unsure of which Windows operating system your computer has, follow the directions on [this](https://support.microsoft.com/en-us/help/13443/windows-which-operating-system) site to check. Refer to [this](https://support.office.com/en-us/article/about-office-what-version-of-office-am-i-using-932788b8-a3ce-44bf-bb09-e334518b8b19) site to determine which version of Microsoft Office is installed.      
...
**NOTE** The databse used in these instructions is named CBIBI_2017, but, as yours will no doubt have a different name, change the code below from `channel <- dbConnect(odbc(), "CBIBI_2017")` to `channel <- dbConnect(odbc(), "your_database_name")`.   
...
```
####**1.4**
Perform method standardizations as described on page 5 of the Final Report by applying the `prep_data()` function from the *bibi2.0* package to the indicated tables. Several functions are nested within `prep_data()`, including:  
...
```
####**1.7** 
Perform probabilistic rarefaction as described on Final Report page 7 and Appendix D.  

The Chessie BIBI uses rarefied counts of 100 because because that was the lowest common denominator among all the agencies and programs represented in the Chessie BIBI database. Changing the sample size to a number other than 100 will alter the final outcome, but if you decide to change the sample size, substitute your number for 100 in the 4^th^ line of code below: `mutate(ratio = YOUR_NUMBER*(reporting_value/sample_sum)) %>% `.  

**WARNING:** every time you rerun the rarefaction scripts below, the resulting data frame will have different rarefied counts for rare species. To preserve the data frame generated by rarefaction, **taxa.df**, export it as a CSV file. If you need to stop and close the document now, import the rarefied dataset CSV table prior to the next step, which is the metric calculations.
```{r}
taxa2.df <- taxa.df %>%                                                                      
  group_by(unique_id) %>%                                                                               
...
Now that the data are in the proper format and the reporting values have been rarefied, the biological metrics can be calculated. The metric formulas are located in Appendix E. Please note that the appendix contains descriptions of all the metrics tested during the development of the Chessie BIBI, so includes several that did not eventually end up as part of the BIBI.     

The scripts below compute all of the metrics included in the Chessie BIBI for each unique sampling event (unique_id). Later on, metrics specific to each bioregion will be selected out.    

The first step is to generate a new data frame for the non-rarefied and rarefied data, both of which are needed because not all metric calculations require rarefied data. Then, a data frame, **metrics.key**, will be generated. **metrics.key** initially only contains the following columns: unique_id, icprb_bioregion_id, event_latitude, event_longitude, and huc_12, but each calculated metric is appended using the `$` notation; for example `metrics.key$new_column <-`.
...
```
####**3.3**
Score the metrics using the approach described on pages 20-21 in the Final Report and outlined below:    

1. Go to Final Report Table 15, Page 44. This table can also be [downloaded](https://www.potomacriver.org/wp-content/uploads/2014/12/metric_thresholds_06292017.csv) from the ICPRB website, but it will need to be filtered so that it only contains bioregion and family data.  
...
Use Table 5, Final Report Page 27 to assign narrative ratings of Excellent, Very Good, Fair, Poor, or Very Poor to the Chessie BIBI values.
```{r}
NAPU_score$rating <-
  case_when(
...
```
To download a CSV of the Chessie BIBI ratings of all bioregions, use the following script. The CSV will be stored in the working directory that you have specified. Don't forget to replace `file= ` with your own file name.
```{r eval=FALSE}
