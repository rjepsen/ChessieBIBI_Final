...
# Calculate the metrics. Use long.df to specify which data frame to use (non-rarefied or rarefied. First, the non-rarefied data will be used.)
 ...
 metrics.key$aspt_mod <- taxa_tol_index( 
  ...
  pct_gastro_oligo <- non_rare %>%
   group_by(unique_id) %>%
     #filter(!is.na(class)) %>% 
      tally(class %in% c("gastropoda", "oligochatea"))
  metrics.key$pct_gastro_oligo <- pct_gastro_oligo$n / sample_sum$sample_sum 
  
  metrics.key$pct_gastro_oligo2 <- taxa_pct(
  long.df = non_rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = class,
...
# Richness/Diversity metrics require rarefied data, so switch the long.df from non_rare to rare.
  ...
  rich_burrow <- rare %>%
    group_by(unique_id) %>%
    filter(!is.na(habit)) %>% 
    tally(habit %in% c("burrow"))
  metrics.key$rich_burrow <- rich_burrow$n
  ...
  rich_climb <- rare  %>%
    ...
    tally(habit %in% c("climb"))
  metrics.key$rich_climb <- rich_climb$n
  
metrics.key$rich_climb2 <- taxa_rich(
  long.df = rare,
  unique.id.col = unique_id,
  low.taxa.col = habit,
  high.taxa.col = family,
